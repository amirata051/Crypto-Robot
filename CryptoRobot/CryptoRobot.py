...
def train_linear_regression_model(coin_data):
    X = np.arange(len(coin_data)).reshape(-1, 1)
    y = coin_data['Price (USD)'].values
...
def plot_linear_regression_forecast(coin_data, model):
    coin_data['Datetime'] = pd.to_datetime(coin_data['Date'] + ' ' + coin_data['Time'])
    plt.figure(figsize=(12, 6))
    plt.plot(coin_data['Datetime'].values, coin_data['Price (USD)'].values, label='Actual Price', color='blue')
    
    forecast_steps = 30
    last_datetime = coin_data['Datetime'].iloc[-1]
    forecast_index = pd.date_range(start=last_datetime, periods=forecast_steps+1)
    forecast_index = forecast_index[1:]  
    X_forecast = np.arange(len(coin_data), len(coin_data) + forecast_steps).reshape(-1, 1)
    forecast = model.predict(X_forecast)
    plt.plot(forecast_index.values, forecast, label='Forecasted Price', color='red')
...
while True:
    ...
    if choice == "1":
        ...
        selected_coin_data = extract_coin_data(selected_coin)
        if not selected_coin_data.empty:
            model = train_linear_regression_model(selected_coin_data)
            plot_linear_regression_forecast(selected_coin_data, model)
        else:
            print(f"No data available for cryptocurrency '{selected_coin}'.")
    elif choice == "7":
        print("Exit.")
